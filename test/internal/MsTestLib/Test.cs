...
namespace MS.Test.Common.MsTestLib
{
    public static class Test
    {
        ...
        public static DateTimeOffset CaseStartTime;
        ...
        public static void Start(string testClass, string testMethod)
        {
            ...
            Test.CaseStartTime = DateTimeOffset.UtcNow;
        }

        public static void End(string testClass, string testMethod)
        {
            TimeSpan executionTime = DateTimeOffset.UtcNow - Test.CaseStartTime;

            if (ErrorCount == 0 && SkipErrorCount == 0)
            {
                Logger.EndTest(testClass + "." + testMethod, executionTime, TestResult.PASS);
            }
            else if (SkipErrorCount > 0)
            {
                SkipCount++;
                Logger.EndTest(testClass + "." + testMethod, executionTime, TestResult.SKIP);
            ...
            {
                FailCount++;
                Logger.EndTest(testClass + "." + testMethod, executionTime, TestResult.FAIL);
