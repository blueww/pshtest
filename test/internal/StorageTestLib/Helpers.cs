...
namespace StorageTestLib
{
    ...
    public static class Helper
    {
        ...
        public static string GenerateRandomTestFile(string filename, int sizeKB)
        {
            byte[] data = new byte[sizeKB * 1024];
            Random r = new Random();
            r.NextBytes(data);
    ...
    /// <summary>
    /// This calss helps do operations on cloud files
    /// </summary>
    public class CloudFileHelper
    {
        ...
        public bool CheckShareExistence(string shareName)
        {
            var share = this.FileClient.GetShareReference(shareName);
            return share.Exists();
        }

        public bool ListShares(string prefix, out IEnumerable<CloudFileShare> shareList)
        {   
            try
            {
                shareList = this.FileClient.ListShares(prefix);
                return true;
            }
        ...
        /// <summary>
        /// Create a directory recursively under a share
        /// </summary>
        /// <param name="shareName">share</param>
        /// <param name="path">relative path to the directory. Accepted value samples: a/b/../d/./e, \a\..\b\</param>
        /// <param name="createRecursively">should create parent directories if they don't exist</param>
        /// <returns></returns>
        public bool CreateDirectory(string shareName, string path, bool createRecursively = true)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
                ...
                var dir = EnterDirectory(
                    root: root,
                    folder: path,
                    createRecursively: createRecursively);
        ...
        /// <summary>
        /// Delete a directory under a share
        /// </summary>
        /// <param name="shareName"></param>
        /// <param name="path"></param>
        /// <returns></returns>
        public bool DeleteDirectory(string shareName, string path, bool shouldCheckAncestorExistence = true)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
                var currentDir = share.GetRootDirectoryReference();
                var pathComponents = ResolvePath(path);

                foreach (var part in pathComponents)
                {
                    currentDir = currentDir.GetDirectoryReference(part);

                    if (shouldCheckAncestorExistence && !currentDir.Exists())
                    {
                        throw new Exception("Ancestor directory does not exist: " + currentDir.Uri);
                    }
                }
        ...
        /// <param name="shareName"></param>
        /// <param name="path"></param>
        public bool List(string shareName, string path, out IEnumerable<IListFileItem> items)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
                var root = share.GetRootDirectoryReference();
                var dir = EnterDirectory(root: root, folder: path);
                items = dir.ListFilesAndDirectories();
        ...
        /// </summary>
        public bool ListFiles(string shareName, string path, out List<CloudFile> files)
        {
            IEnumerable<IListFileItem> items;
            files = new List<CloudFile>();
            if (!List(shareName, path, out items))
            {
                return false;
            }
        ...
        public bool CreateFile(string shareName, string folder, string filename, long size = 0)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
                ...
                var root = share.GetRootDirectoryReference();
                var dir = EnterDirectory(root, folder);
        ...
        public bool GetFileProperties(string shareName, string folder, string filename, out FileProperties properties)
        {
            properties = null;
            try
            {
        ...
        {
            return DeleteFiles(shareName, folder: ".");
        }

        public bool DeleteFiles(string shareName, string folder, string filename = null)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
        ...
        public CloudFile QueryFile(string shareName, string folder, string fileName)
        {
            try
            {
                var share = this.FileClient.GetShareReference(shareName);
                if (!share.Exists())
                {
                    throw new Exception("Share " + shareName + " does not exist");
                }
                ...
                if (!file.Exists())
                {
                    throw new Exception("File " + fileName + " does not exist in share " + shareName);
                }
        ...
        /// <summary>
        /// Get into a subfolder specified by path. Additional option createRecursively is to create intermediate folders.
        /// </summary>
        /// <param name="root"></param>
        /// <param name="folder"></param>
        /// <param name="createRecursively"></param>
        /// <returns></returns>
        private CloudFileDirectory EnterDirectory(CloudFileDirectory root, string folder, bool createRecursively = false)
        {
            var currentDir = root;
            var pathComponents = ResolvePath(folder);
        ...
        /// <summary>
        /// Delete all files and subdirectories recursively, but keep the passed-in directory.
        /// </summary>
        /// <returns></returns>
        private void DeleteFolderRecursively(CloudFileDirectory dir)
        {
            ...
            foreach (var item in items)
            {
                ...
                throw new Exception("I don't know what it is. Item type: " + item.GetType());
            }
        }

        /// <summary>
        /// resolve a relative path to path componenets
        /// </summary>
        /// <param name="relativePath">relative path to resolve. Accepted value samples: /a/b/.././../d</param>
        /// <returns></returns>
        private string[] ResolvePath(string relativePath)
        {
            ...
            relativePath.Replace('\\', '/').Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries).ToList().ForEach(
                    part =>
                    {
                        if (part == "..")
                        {
    ...
    public class CloudBlobHelper
    {
        ...
        public CloudBlob QueryBlob(string containerName, string blobName)
        {
            ...
            {
                CloudBlobContainer container = BlobClient.GetContainerReference(containerName);
                CloudBlob blob = StorageExtensions.GetBlobReferenceFromServer(container, blobName);
                return blob;
            }
            catch (StorageException e)
            {
        ...
        {
            if (cbd == null) return;
            foreach (CloudBlob blob in cbd.ListBlobs(true, BlobListingDetails.All))
            {
                blob.Delete();
            }
        ...
        public bool UploadFileToBlockBlob(string containerName, string blobName, string filePath)
        {
            try
            {
                ...
                Test.Info("Blob {0} has been uploaded successfully.", blobName);
                return true;
            }
            catch (StorageException e)
            {
                if (null == e ||
                    ...
                    BlobErrorCodeStrings.BlobNotFound == e.RequestInformation.ExtendedErrorInformation.ErrorCode)
                {               
                    Test.Error("CloudBlobHelper.UploadFileToBlockBlob: {0}", e.ToString());
                    return false;
                }
                throw;
            }
        ...
        public bool UploadFileToAppendBlob(string containerName, string blobName, string filePath)
        {
            try
            {
                ...
                using (FileStream fileStream = new FileStream(Path.Combine(filePath), FileMode.Open))
                {
                    appendBlob.UploadFromStream(fileStream, null, bro);
                    fileStream.Close();
                }
                Test.Info("Blob {0} has been uploaded successfully.", blobName);
                return true;
        ...
        public bool UploadFileToPageBlob(string containerName, string blobName, string filePath)
        {
            ...
            catch (StorageException e)
            {
                if (null == e ||
                    ...
                    BlobErrorCodeStrings.BlobNotFound == e.RequestInformation.ExtendedErrorInformation.ErrorCode)
                {
                    Test.Error("CloudBlobHelper.UploadfileToPageBlob: {0}", e.ToString());
                    return false;
                }
                throw;
            }
            ...
            try
            {
                ...
                switch (blob.Properties.BlobType)
                {
                    case StorageBlobType.BlockBlob:
                        return (blob as CloudBlockBlob).CreateSnapshot();
                    case StorageBlobType.PageBlob:
                        return (blob as CloudPageBlob).CreateSnapshot();
                    case StorageBlobType.AppendBlob:
                        return (blob as CloudAppendBlob).CreateSnapshot();
                    default: 
                        throw new InvalidOperationException(string.Format("Does not support blob type: {0}", blob.Properties.BlobType));
                }
            }
            catch (StorageException e)
            {
