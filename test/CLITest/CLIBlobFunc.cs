    ...
    public class CLIBlobFunc : TestBase
    {
        ...
        internal void GetNonExistingBlob(Agent agent)
        {
            ...
            try
            {
                ...
                bool expectedError = agent.ErrorMessages[0].Contains(String.Format("Can not find blob '{0}' in container '{1}', or the blob type is unsupported.", BLOB_NAME, CONTAINER_NAME))
